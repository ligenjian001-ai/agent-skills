...
**整合后**：一个 `task-delegate` skill，统一管理所有后端的委派、监控、验证和复盘。`task_launch.sh` 是唯一的 dispatch 层，所有编排技能（如 panel-discussion）通过 thin wrapper 调用它。
...
| `scripts/task_launch.sh` | 统一启动器（tmux session + runner + 多后端 + --role/--source/--trace） |
| `scripts/task_monitor.sh` | 任务状态监控（列表 + 详情） |
| `scripts/ag_trace.py` | Langfuse 异步追踪（fire-and-forget，通过 --trace 启用） |
