...
单一 AI agent 分析 bug 时容易陷入思路固定（tunnel vision）。一个 agent 可能过于关注代码层面的表象，忽略逻辑层面的假设错误；反之亦然。实际使用中观察到，单 agent 独立分析 bug 给出错误答案的概率不低。

## The Solution

**双分析师独立诊断 + AG 综合裁决**：

1. Codex 作为**代码分析师**——擅长代码路径追踪、数据流分析
...
两个 Codex agent 通过不同的 prompt 模板差异化：一个专注代码路径追踪，一个专注逻辑推理。Codex 的推理能力强，适合两种分析角色。Prompt 进一步强化了这种分工。
...
| `prompts/code_analyst.txt` | Codex 代码分析师角色模板 |
...
| Date | Version | Change |
|------|---------|--------|
| 2026-03-09 | v1.0 | Initial creation — dual analyst (Codex × 2) investigation |
