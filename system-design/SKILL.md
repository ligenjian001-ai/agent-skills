...
| Agent | 后端 | 视角 | 产出重点 |
|-------|------|------|---------|
| 🏛 Architect | Codex | 纯架构设计：组件、接口、交互模式 | **关键设计选择及推荐理由** |
| 🔧 Realist | Codex | 基于现有代码的改造路径 | **哪些能复用、哪些必须重写、风险在哪里** |
...
bash ~/agent-skills/task-delegate/scripts/task_launch.sh \
  ${DESIGN_ID}_architect ${PROJECT_DIR} --backend codex \
...
> [!CAUTION]
> **禁止自验模式。** "AG 设计实验 → Codex 执行 → AG 审查"是无效验证。
...
bash ~/agent-skills/task-delegate/scripts/task_launch.sh \
  ${DESIGN_ID}_spike001 ${PROJECT_DIR} --backend codex \
...
| 选项 | 适用场景 |
|------|---------|
| **立即执行** | 当前对话直接进入执行，AG 委派 Codex 逐步实现 |
...
- **区分编码/验证角色** — 编码用 Codex，验证用独立 agent 或用户
